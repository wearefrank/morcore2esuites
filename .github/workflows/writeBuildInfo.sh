...
if [[ -n $1 ]]; then
  ...
  echo "configuration.version=$1" > configurations/MessageProcessor/BuildInfo.properties
  echo "configuration.timestamp=$(date +%Y%m%d-%H%M%S)" >> configurations/MessageProcessor/BuildInfo.properties
